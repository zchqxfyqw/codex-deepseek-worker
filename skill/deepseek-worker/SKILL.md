...
- Consume `status.json`, `summary.txt`, `changed-files.txt`, `diff-stat.txt`, and `commands.json` first. Process exit, timeout, cleanup, and Git hard boundaries determine `runner_state`; command evidence supports task acceptance but does not alone rewrite that state. `summary.txt` is non-authoritative plain text or Markdown. Treat overlap with pre-existing dirty files as a review warning, not automatic failure.
